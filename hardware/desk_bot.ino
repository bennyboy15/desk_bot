#include "U8glib.h"

U8GLIB_SH1106_128X64 u8g(13, 11, 10, 9); // SCK, MOSI, CS, DC(A0)

// Eye geometry
const int eyeWidth = 34;
const int eyeHeight = 40;
const int eyeRadius = 10;
const int eyeGap = 14;
const int centerY = 32;

// Base eye positions (left eye, right eye mirrors)
int baseLeftX = 64 - eyeGap / 2 - eyeWidth;
int baseRightX = 64 + eyeGap / 2;

// Current offsets for "look around" drift
int offsetX = 0;
int offsetY = 0;

// Current eye openness (height), shared with draw()
int currentHeight = eyeHeight;

// Blink timing
unsigned long lastBlinkTime = 0;
unsigned long nextBlinkDelay = 3000;

// Look-around timing
unsigned long lastLookTime = 0;
unsigned long nextLookDelay = 2000;

// Face selection, driven by serial input
enum Mood
{
    NEUTRAL,
    HAPPY,
    SAD,
    WINK,
    THINK
};
Mood mood = NEUTRAL;

void drawNeutralEye(int x)
{
    int y = centerY - currentHeight / 2 + offsetY;
    u8g.drawRBox(x + offsetX, y, eyeWidth, currentHeight, eyeRadius);
}

// Upward crescent: full disc with a lower disc erased out of it
void drawHappyEye(int x)
{
    int cx = x + eyeWidth / 2;
    int r = eyeWidth / 2;
    u8g.drawDisc(cx, centerY, r);
    u8g.setColorIndex(0);
    u8g.drawDisc(cx, centerY + 12, r + 3);
    u8g.setColorIndex(1);
}

// Normal eye with the top-outer corner cut away by a drooping eyelid
void drawSadEye(int x, bool isLeft)
{
    int y = centerY - eyeHeight / 2;
    u8g.drawRBox(x, y, eyeWidth, eyeHeight, eyeRadius);
    u8g.setColorIndex(0);
    if (isLeft)
    {
        u8g.drawTriangle(x - 1, y - 1,
                         x + eyeWidth + 1, y - 1,
                         x - 1, y + eyeHeight / 2);
    }
    else
    {
        u8g.drawTriangle(x - 1, y - 1,
                         x + eyeWidth + 1, y - 1,
                         x + eyeWidth + 1, y + eyeHeight / 2);
    }
    u8g.setColorIndex(1);
}

// Closed eye: a thin horizontal bar
void drawClosedEye(int x)
{
    int h = 6;
    u8g.drawRBox(x, centerY - h / 2, eyeWidth, h, 3);
}

// Half-closed eye shifted to look up and to the side
void drawThinkEye(int x)
{
    int h = eyeHeight / 2;
    u8g.drawRBox(x + 10, centerY - h - 4, eyeWidth, h, 6);
}

void draw(void)
{
    switch (mood)
    {
    case HAPPY:
        drawHappyEye(baseLeftX);
        drawHappyEye(baseRightX);
        break;
    case SAD:
        drawSadEye(baseLeftX, true);
        drawSadEye(baseRightX, false);
        break;
    case WINK:
        drawNeutralEye(baseLeftX);
        drawClosedEye(baseRightX);
        break;
    case THINK:
        drawThinkEye(baseLeftX);
        drawThinkEye(baseRightX);
        break;
    case NEUTRAL:
    default:
        drawNeutralEye(baseLeftX);
        drawNeutralEye(baseRightX);
        break;
    }
}

void render()
{
    u8g.firstPage();
    do
    {
        draw();
    } while (u8g.nextPage());
}

void doBlink()
{
    // Close
    for (int h = eyeHeight; h >= 4; h -= 8)
    {
        currentHeight = h;
        render();
        delay(15);
    }
    delay(60);
    // Open
    for (int h = 4; h <= eyeHeight; h += 8)
    {
        currentHeight = h;
        render();
        delay(15);
    }
    currentHeight = eyeHeight;
    render();
}

void updateLookOffset()
{
    offsetX = random(-6, 7);
    offsetY = random(-4, 5);
}

void setMood(char c)
{
    switch (c)
    {
    case 'h': mood = HAPPY; break;
    case 's': mood = SAD; break;
    case 'w': mood = WINK; break;
    case 't': mood = THINK; break;
    case 'n': mood = NEUTRAL; break;
    default: 
        mood = NEUTRAL;
        return; // ignore anything else (e.g. newlines)
    }
    // Reset idle animation state so neutral comes back clean
    offsetX = 0;
    offsetY = 0;
    currentHeight = eyeHeight;
    Serial.print("mood: ");
    Serial.println(c);
}

void setup(void)
{
    // FACES SETUP
    randomSeed(analogRead(A0));
    nextBlinkDelay = random(2000, 5000);
    nextLookDelay = random(1500, 4000);

    if (u8g.getMode() == U8G_MODE_BW)
    {
        u8g.setColorIndex(1); // pixel on
    }

    // SERIAL COMMUNICATION
    Serial.begin(9600);
    mood = NEUTRAL;
}

void loop(void)
{
    // READ INCOMING SERIAL COMMUNICATION
    if (Serial.available() > 0)
    {
        char c = Serial.read();
        setMood(tolower(c));
    }

    unsigned long now = millis();

    // Idle animation (drift + blink) only applies to the neutral face;
    // the other faces hold still until a new command comes in.
    if (mood == NEUTRAL)
    {
        if (now - lastLookTime > nextLookDelay)
        {
            updateLookOffset();
            lastLookTime = now;
            nextLookDelay = random(1500, 4000);
        }
        render();
        if (now - lastBlinkTime > nextBlinkDelay)
        {
            doBlink();
            if (random(0, 100) < 25)
            {
                delay(120);
                doBlink();
            }
            lastBlinkTime = now;
            nextBlinkDelay = random(2000, 5000);
        }
    }
    else
    {
        render();
    }
}
